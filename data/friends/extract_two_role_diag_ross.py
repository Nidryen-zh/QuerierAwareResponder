...
def get_role_list(diags):
    def get_role_pinyin(role):
        output = role.replace(" ", "").replace(".", "")
        return output
    ...
    # filter out roles whose conversation is less than 70
    role_list = []
    for role in role_dict:
        if role_dict[role] >= 70:
            role_list.append(role)    
    role_pinyin = {role: "".join(get_role_pinyin(role)) for role in role_list}
...
if __name__ == "__main__":
    ...
    os.makedirs("diags_two_role_ross/configs", exist_ok=True)
    json.dump(sorted_role_dict, open("diags_two_role_ross/configs/role_dict.json", 'w', encoding='utf-8'), ensure_ascii=False, indent=4)
    ...
    for role2 in role_pinyin:
        ...
        if len(new_diags) < 20:
            ...
            continue
        output_dir = f'diags_two_role_ross/{role1_pinyin}_{role2_pinyin}'
    ...
    json.dump(role_pinyin, open("diags_two_role_ross/configs/role_list.json", 'w', encoding='utf-8'), ensure_ascii=False, indent=4)
    # Step 2-5: extract diag for target role
    role_pair_id = {role1: {role2: i+1 for i, role2 in enumerate(role_pinyin)}}
    json.dump(role_pair_id, open("diags_two_role_ross/configs/role_pair_id.json", 'w', encoding='utf-8'), ensure_ascii=False, indent=4)
    for role2 in role_pinyin: 
        role2_pinyin = role_pinyin[role2]
        input_dir = f'diags_two_role_ross/{role1_pinyin}_{role2_pinyin}'
