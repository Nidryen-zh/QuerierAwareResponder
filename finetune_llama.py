...
def preprocess_multirole_eval(sources, target_role, tokenizer, max_len, system_message: str = SYSTEM, role_pair_id: int = None):
    ...
    im_end = tokenizer("<|eot_id|>").input_ids[1:] ### <|end_of_text|> or <|eot_id|> or no im_end ?
    nl_tokens = tokenizer('\n').input_ids[1:]
    _system = tokenizer('system').input_ids[1:] + nl_tokens
    _target_role = im_start + tokenizer(target_role).input_ids[1:] + nl_tokens
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            if role == target_role and j == len(source) - 1:
                ...
                _input_id = im_start + tokenizer(role).input_ids[1:] + nl_tokens
            else:
                _input_id = im_start + tokenizer(role).input_ids[1:] + nl_tokens + tokenizer(sentence["value"]).input_ids[1:] + im_end + nl_tokens
            if role == target_role:
                if j == len(source) - 1:
        ...
        target_id = tokenizer(target).input_ids[1:]
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            if role != target_role:
                ...
                contain_target_role = True
                _target = im_start + [IGNORE_TOKEN_ID] * (len(tokenizer(role).input_ids[1:]) + 1) + _input_id[len(tokenizer(role).input_ids[1:]) + 2:-2] + im_end + nl_tokens
            if role == target_role:
                pos_id += [0] + [1] * (len(_input_id) - 3) + [0] + [0]
            else:
                pos_id += [0] + [role_pair_id] * (len(_input_id) - 3) + [0] + [0]
...
def train():
    ...
    if training_args.role_decomposition:
        ...
        model = LlamaForCausalLMRoleDecomposition.from_pretrained(
            model_args.model_name_or_path,
            config=config,
            torch_dtype="auto",
            cache_dir=training_args.cache_dir,
