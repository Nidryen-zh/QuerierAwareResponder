...
from utils import DataCollatorWithPaddingAndPreserveString
...
class Trainer:
    ...
    def __init__(
        ...
        # transformers==4.37.0 version
        default_collator = default_data_collator if tokenizer is None else DataCollatorWithPadding(tokenizer)
        # my version
        # default_collator = default_data_collator if tokenizer is None else DataCollatorWithPaddingAndPreserveString(tokenizer)
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if not isinstance(self.model, supported_classes):
            ...
            if self.args.save_safetensors:
                safetensors.torch.save_file(
                    state_dict, os.path.join(output_dir, "full_parameters.safetensors"), metadata={"format": "pt"}
                )
            else:
                torch.save(state_dict, os.path.join(output_dir, "full_parameters.pth"))
    ...
    def evaluate(
        ...
        ## output: EvalLoopOutput(predictions=None, label_ids=None, metrics={'eval_loss': 10.86}, num_samples=1453)
        output = eval_loop(
            eval_dataloader,
    ...
    def evaluation_loop(
        ...
        ## inputs: {'inputs_ids': tensor([[xx]]), 'labels': tensor([[xx]]), 'attention_mask': tensor([[xx]])}
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            ## logits: None, loss: tensor(xx), labels: None
