'''
[NOTE]
fork from transformers.modeling_qwen2
transformers==4.37.0
'''
# coding=utf-8
...
'''
class Qwen2MLPRoleDecomposition(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class Qwen2MLPRoleDecompositionLowRank(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size  # hidden_size: 4096
        self.rank = 16  # larger rank: 16 -> 64 # intermediate_size: 11008
...
class Qwen2DecoderLayerRoleDecomposition(Qwen2DecoderLayer):
    def __init__(self, config: Qwen2Config, layer_idx: int, decomposition: bool = True, full_para: bool = False):
        super().__init__(config, layer_idx)

        self.decomposition = decomposition
        self.mlp = Qwen2MLPRoleDecomposition(config)
        self.full_para = full_para
        self.layer_idx = layer_idx

        if self.decomposition:
            if self.full_para:
                self.mlp_specific = Qwen2MLPRoleDecomposition(config)
            else:
                self.mlp_specific = Qwen2MLPRoleDecompositionLowRank(config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        residual = hidden_states
        hidden_states = self.post_attention_layernorm(hidden_states)
        # hidden_states, hidden_states_general, hidden_states_specific = self.mlp(hidden_states)
        hidden_states = self.mlp(hidden_states)
        hidden_states = residual + hidden_states
...
class Qwen2ModelRoleDecomposition(Qwen2Model):
    def __init__(self, config: Qwen2Config, decomposition=True, need_role_emb=True, no_general_role_emb=False, full_para=False):
        super().__init__(config)
        ...
        if self.need_role_emb:
            ...
            # self.role_embed_tokens.weight.data = torch.zeros(self.role_embed_tokens.weight.data.shape).to(self.device).to(self.role_embed_tokens.weight.data.dtype)

        self.layers = nn.ModuleList(
            [Qwen2DecoderLayerRoleDecomposition(config, layer_idx, decomposition, full_para) for layer_idx in range(config.num_hidden_layers)]
        )

        self.norm_specific = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.norm_role_proj = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            ...
            if self.decomposition:
                ...
                # hidden_states_specific_temp = hidden_states_specific * attention_mask_raw.reshape((B, -1, 1))
        ...
        next_cache = None
        if use_cache:
            next_cache = next_decoder_cache.to_legacy_cache() if use_legacy_cache else 0
...
class Qwen2ForCausalLMRoleDecomposition(Qwen2ForCausalLM):
    def __init__(self, config, decomposition=True, need_role_emb=True, only_last_layer_loss=False, only_middle_layer_loss=False,
                 ...
                 use_cluster_specific_mlp=False, use_cluster_specific_role_proj=False, pretrain_role_proj_step=0, average_role_token_as_emb=True, use_role_proj=False, role_proj_dim=512, lam=0.5, K=512, T=0.01, cluster_num=10,
                 output_dir="temp"):
        super().__init__(config, output_dir)
        print("[DEBUG] init Qwen2ForCausalLMRoleDecomposition: need_role_emb: {}; only_last_layer_loss: {}; use_role_cls: {}; decomposition_pretrain: {}; no_general_role_emb: {}; only_push: {}; weighted_decomposition: {}; NP_cls: {}".format(need_role_emb, only_last_layer_loss, use_role_cls, decomposition_pretrain, no_general_role_emb, only_push, weighted_decomposition, NP_cls, K, T, NP_cls))
        print("[DEBUG] init Qwen2ForCausalLMRoleDecomposition: use_role_proj: {} role_proj_dim: {} average_role_token_as_emb: {} pretrain_role_proj_step: {} T: {} use_cluster_specific_role_proj: {}".format(use_role_proj, role_proj_dim, average_role_token_as_emb, pretrain_role_proj_step, T, use_cluster_specific_role_proj))
        print("[DEBUG] init Qwen2ForCausalLMRoleDecomposition: K: {} cluster_num: {}".format(K, cluster_num))

        self.model = Qwen2ModelRoleDecomposition(config, decomposition, need_role_emb, no_general_role_emb, full_para)
    ...
    def forward(
        ...
        if self.decomposition:
            ...
            if self.weighted_decomposition:
                ...
                # if torch.distributed.get_rank() == 0:
                #     print(sig[0][10:20].squeeze(1).tolist())
